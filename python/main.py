""" Mouse Fatigue Estimation by GSR and EMG Values w/ TensorFlow
 ...
 Collate forearm muscle soreness data on the SD card
 build and train a neural network model
 and run the model directly on Wio Terminal.
...
class Mouse_Fatigue:
    """ Create a class to build a neural network model after visualizing
        and scaling (normalizing) the soreness data (GSR and EMG)
        collected by Wio Terminal."""
    def __init__(self, csv_path):
        ...
        self.dt_frm = pd.read_csv(csv_path)
    ...
    def graphics(self, column_1, column_2, x_label, y_label):
        ...
        plt.gcf().canvas.set_window_title(
                'Mouse Fatigue Estimation by GSR and EMG Values')
        plt.hist2d(
                self.dt_frm[column_1], self.dt_frm[column_2], cmap="coolwarm")
        plt.colorbar()
        plt.xlabel(x_label)
        plt.ylabel(y_label)
        plt.title(x_label)
    ...
    def data_visualization(self):
        """  Visualize data before creating and
             training the neural network model.
             Scrutinize data columns to build a model
             with appropriately formatted data
        """
    ...
    def scale_data_and_define_inputs(self):
        ...
        self.dt_frm["scaled_GSR"] = self.dt_frm["GSR"] / self.scale_val
        self.dt_frm["scaled_EMG"] = self.dt_frm["EMG"] / self.scale_val
        # Create the inputs array by utilizing the scaled variables:
        for i in range(len(self.dt_frm)):
            self.inputs.append(np.array(
                [self.dt_frm["scaled_GSR"][i], self.dt_frm["scaled_EMG"][i]]))
        self.inputs = np.asarray(self.inputs)

    def define_and_assign_labels(self):
        """ Assign labels for each input according to the predefined soreness
            classes for each data record.
        """
